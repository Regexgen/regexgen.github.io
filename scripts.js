...
$(document).ready(function() {
	...
	function start2() {
		...
		text = text.replace(/([0-9])/gi,"<span class='nam'>$1</span>");
		text = text.replace(/(a)/i,"[<span class='lat'>a</span><span class='cyr'>а</span>]");
		text = text.replace(/(b)/i,"[<span class='lat'>b</span><span class='cyr'>б</span><span class='nam'>6</span>]");
		text = text.replace(/(c)/i,"[<span class='lat'>cs</span><span class='cyr'>с</span>]");
		text = text.replace(/(d)/i,"[<span class='lat'>d</span><span class='cyr'>д</span>]");
		text = text.replace(/(e)/i,"[<span class='lat'>e</span><span class='cyr'>е</span>]");
		text = text.replace(/(f)/i,"[<span class='lat'>f</span><span class='cyr'>ф</span>]");
		text = text.replace(/(g)/i,"[<span class='lat'>g</span><span class='cyr'>г</span>]");
		text = text.replace(/(h)/i,"[<span class='lat'>h</span><span class='cyr'>хн</span>]");
		text = text.replace(/(i)/i,"[<span class='lat'>iu</span><span class='cyr'>и</span>]");
		text = text.replace(/(j)/i,"[<span class='lat'>j</span><span class='cyr'>ж</span>]");
		text = text.replace(/(k)/i,"[<span class='lat'>k</span><span class='cyr'>к</span>]");
		text = text.replace(/(l)/i,"[<span class='lat'>l</span><span class='cyr'>л</span>]");
		text = text.replace(/(m)/i,"[<span class='lat'>m</span><span class='cyr'>м</span>]");
		text = text.replace(/(n)/i,"[<span class='lat'>nh</span><span class='cyr'>н</span>]");
		text = text.replace(/(o)/i,"[<span class='lat'>o</span><span class='cyr'>о</span><span class='nam'>0</span>]");
		text = text.replace(/(p)/i,"[<span class='lat'>p</span><span class='cyr'>рп</span>]");
		text = text.replace(/(q)/i,"<span class='lat'>q</span>");
		text = text.replace(/(r)/i,"[<span class='lat'>r</span><span class='cyr'>гр</span>]");
